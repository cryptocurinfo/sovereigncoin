...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("SVC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("sovereigncoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SVC unit input in lnReqAmount, we generate the URI with SVC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SVC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
